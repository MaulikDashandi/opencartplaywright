...
- Name: LoginDataDriven.spec.ts >> Login Test with JSON Data: Invalid login
- Location: tests\LoginDataDriven.spec.ts:17:9
...
```ts
  ...
  7  | import LoginData from '../testdata/logindata.json';
  8  | 
  ...
  16 | {
  17 |     test(`Login Test with JSON Data: ${data.testName}`,{tag:['@datadriven']},async ({page})=>{
  18 |         
